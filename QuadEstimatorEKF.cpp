...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  while ( (ekfState(6) > F_PI) || (ekfState(6) < -F_PI) ) {
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  predictedState(3) += accel.x * dt;
  predictedState(4) += accel.y * dt;
  predictedState(5) += (accel.z - 9.81) * dt;
  /////////////////////////////// END STUDENT CODE ////////////////////////////
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  float e1 = z(0) - zFromX(0);
  float e2 = z(0) + 2*PI - zFromX(0);
  float e3 = z(0) - 2*PI - zFromX(0);
  float abs_e_min = min(fabs(e1), min(fabs(e2), fabs(e3)));
    
  // Recalculate zFromX so that the chosen error satisfies
  // error = z(0) - zFromX(0).
  if (abs_e_min == fabs(e1)) {
    zFromX(0) = z(0) - e1;
  } else if (abs_e_min == fabs(e2)) {
    zFromX(0) = z(0) - e2;
  } else if (abs_e_min == fabs(e3)) {
    zFromX(0) = z(0) - e3;
  }
  /////////////////////////////// END STUDENT CODE ////////////////////////////
